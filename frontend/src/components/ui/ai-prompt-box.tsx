...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-[50%] top-[50%] z-50 grid w-full max-w-[90vw] md:max-w-[800px] translate-x-[-50%] translate-y-[-50%] gap-4 border border-[#333333] bg-[#1F2023] p-0 shadow-xl rounded-2xl duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out",
        className
      )}
      {...props}
    >
...
const VoiceRecorder: React.FC<{ isRecording: boolean; onStartRecording: () => void; onStopRecording: (d: number) => void; visualizerBars?: number }> = ({
...
const PromptInput = React.forwardRef<HTMLDivElement, { isLoading?: boolean; value?: string; onValueChange?: (v: string) => void; maxHeight?: number | string; onSubmit?: () => void; children: React.ReactNode; className?: string; disabled?: boolean; onDragOver?: (e: React.DragEvent) => void; onDragLeave?: (e: React.DragEvent) => void; onDrop?: (e: React.DragEvent) => void }>(
...
const PromptInputTextarea: React.FC<{ disableAutosize?: boolean; placeholder?: string } & React.ComponentProps<typeof Textarea>> = ({ className, onKeyDown, disableAutosize = false, placeholder, ...props }) => {
  ...
  return (
    <Textarea ref={textareaRef} value={value} onChange={(e) => setValue(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); onSubmit?.(); } onKeyDown?.(e); }} className={cn("text-base", className)} disabled={disabled} placeholder={placeholder} {...props} />
  );
};
...
export const PromptInputBox = React.forwardRef<HTMLDivElement, PromptInputBoxProps>((props, ref) => {
  const { onSend = () => {}, isLoading = false, placeholder = "Ask a question...", className } = props;
  ...
  const handleSubmit = () => {
    ...
    let msg = input;
    if (showSearch) msg = `[Search: ${input}]`;
    else if (showThink) msg = `[Think: ${input}]`;
    else if (showCanvas) msg = `[Canvas: ${input}]`;
    onSend(msg, files);
    setInput("");
    setFiles([]);
    setFilePreviews({});
  ...
  return (
    <>
      ...
      >
        ...
        <PromptInputActions className="justify-between p-0 pt-2">
          <div className={cn("flex items-center gap-1 transition-opacity duration-300", isRecording ? "opacity-0 invisible h-0" : "opacity-100 visible")}>
            ...
            <div className="flex items-center">
              {[
                { key: "search", icon: Globe, color: "#1EAEDB", bg: "#1EAEDB", label: "Search", active: showSearch, toggle: () => { setShowSearch((p) => !p); setShowThink(false); } },
                { key: "think", icon: BrainCog, color: "#8B5CF6", bg: "#8B5CF6", label: "Think", active: showThink, toggle: () => { setShowThink((p) => !p); setShowSearch(false); } },
                { key: "canvas", icon: FolderCode, color: "#F97316", bg: "#F97316", label: "Canvas", active: showCanvas, toggle: () => setShowCanvas((p) => !p) },
              ].map(({ key, icon: Icon, color, label, active, toggle }, idx) => (
                <React.Fragment key={key}>
                  {idx > 0 && <CustomDivider />}
                  <button type="button" onClick={toggle}
                    className={cn("rounded-full transition-all flex items-center gap-1 px-2 py-1 border h-8",
                      active ? `bg-[${color}]/15 border-[${color}] text-[${color}]` : "bg-transparent border-transparent text-[#9CA3AF] hover:text-[#D1D5DB]"
                    )}
                    style={active ? { background: `${color}26`, borderColor: color, color } : undefined}
                  >
                    <motion.div animate={{ rotate: active ? 360 : 0, scale: active ? 1.1 : 1 }} transition={{ type: "spring", stiffness: 260, damping: 25 }}>
                      <Icon className="w-4 h-4" />
                    </motion.div>
                    <AnimatePresence>
                      {active && (
                        <motion.span initial={{ width: 0, opacity: 0 }} animate={{ width: "auto", opacity: 1 }} exit={{ width: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="text-xs overflow-hidden whitespace-nowrap flex-shrink-0">
                          {label}
                        </motion.span>
                      )}
                    </AnimatePresence>
